...
const port = Number(process.env.COMMERCIAL_E2E_PORT ?? 3100);
let baseURL = process.env.COMMERCIAL_E2E_BASE_URL ?? `http://localhost:${port}`;
...
// Prefer localhost (not 127.0.0.1) so browser origin matches Next's default host
// and client components / server actions hydrate correctly.
if (!skipWebServer && !process.env.COMMERCIAL_E2E_BASE_URL) {
