...
import { getScoreBand, type ScoreBand } from "./score-bands";
...
export function getCategoryScorecard(
  ...
  return REPORT_CATEGORY_DISPLAY_ORDER.flatMap((category) => {
    ...
    if (!score || score.maxScore <= 0 || score.applicable === false) {
      return [];
    }
...
export { SCORE_BANDS, getScoreBand } from "./score-bands";
export type { ScoreBand } from "./score-bands";
