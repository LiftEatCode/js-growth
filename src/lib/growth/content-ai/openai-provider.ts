...
export type ContentAiProviderResult = {
  parsed: unknown;
  model: string;
  usage: {
...
export type ContentAiProvider = {
  generate(input: {
    ...
    user: string;
    timeoutMs: number;
  }): Promise<ContentAiProviderResult>;
};
...
export function createContentAiProvider(options: {
  apiKey: string | null;
}): ContentAiProvider {
  return {
        ...
        const response = await client.responses.create(
          {
            ...
            input: input.user,
            ...modelParams,
            max_output_tokens: Math.min(modelParams.max_output_tokens, 8_000),
            text: {
              format: zodTextFormat(
                contentDraftStructuredSchema,
                "content_draft",
              ),
            },
          },
