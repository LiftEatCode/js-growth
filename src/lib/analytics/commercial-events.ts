...
const FORBIDDEN_ANALYTICS_PARAM_KEY_PATTERN =
  /report[_-]?id|session[_-]?id|payment[_-]?intent|customer[_-]?email|^email$|^url$|website_url|competitor[_-]?url|lead[_-]?id|contact[_-]?email|prospect[_-]?email|^to_email$|outreach[_-]?status|outcome[_-]?notes|prospect[_-]?id|contact[_-]?form[_-]?url|contact[_-]?form[_-]?id|outreach[_-]?channel|submitted[_-]?by[_-]?email|provider[_-]?message[_-]?id|delivery[_-]?status|bounce[_-]?reason|webhook[_-]?id|svix[_-]?id|competitor[_-]?id|competitor[_-]?hostname|provider[_-]?business[_-]?id|place[_-]?id|^latitude$|^longitude$|^lat$|^lng$|coordinates/i;
