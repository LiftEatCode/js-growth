...
export interface ImplementationInterpretationProviderResult {
  parsed: unknown;
  model: string;
  usage: {
...
export type ImplementationInterpretationProvider = {
  generate(input: {
    model: string;
    system: string;
    user: string;
    timeoutMs: number;
  }): Promise<ImplementationInterpretationProviderResult>;
};
...
export function createOpenAiImplementationInterpretationProvider(options: {
  apiKey: string | null;
}): ImplementationInterpretationProvider {
  return {
