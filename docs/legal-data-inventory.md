...
- **How collected:** bounded fetch of the business homepage plus up to four additional same-host contact/about/team pages that are actually linked. Maximum five pages per prospect. No social-profile scraping, no third-party enrichment provider, no WHOIS harvesting. Competitor website audits (Sprint 10) reuse the same secure public-website fetch/crawl path as Website Growth Audits for human-selected competitor URLs only.
- **Storage:** PostgreSQL via Prisma (`ProspectContact`, `ProspectContactForm`, `OutreachMessage`, `OutreachDeliveryEvent`, `ProspectContactDiscoveryRun`, `ProspectOutreachDraftRun`, `ProspectCompetitor`, `CompetitorDiscoveryRun`, `CompetitorAudit`, `CompetitorAuditRun`, `CompetitiveComparisonSnapshot`). Contact-form records store public form URL/metadata only. Delivery events store provider message IDs, event types, timestamps, and minimal safe metadata — not full webhook payloads or email bodies. Competitor audits store Website Growth Audit snapshots for human-selected competitor websites only; they are not `AuditReport` records and are not exposed on public report pages, PDFs, Professional APIs, Stripe flows, AI Interpretation, or analytics events. Competitive comparison snapshots store deterministic gap/advantage JSON derived only from stored audits (no additional website crawling). These records are available only in the authenticated `/reports/prospecting` workspace.
...
- **Potential data:** pages viewed, browser/device characteristics, approximate geography, referral/source information, and interaction events. Custom events include commercial and growth funnel events (e.g. `audit_completed`, `audit_started`, `audit_submitted`, `contact_form_submitted`, `professional_checkout_started`, multi-page/competitive/AI events with bounded params). Report UUIDs, Stripe identifiers, emails, phones, and commercial record IDs are not included in custom events. Report routes send a sanitized `page_path` such as `/report/[id]` instead of the capability UUID. Proposal and agreement share links send sanitized paths `/proposal/[secure]` and `/agreement/[secure]` — raw access tokens never leave the browser URL bar into GA4. Payment return pages omit `session_id` and related Stripe query parameters from analytics locations.
