...
- **Storage:** PostgreSQL via Prisma (`ProspectContact`, `ProspectContactForm`, `OutreachMessage`, `OutreachDeliveryEvent`, `ProspectContactDiscoveryRun`, `ProspectOutreachDraftRun`, `ProspectCompetitor`, `CompetitorDiscoveryRun`, `CompetitorAudit`, `CompetitorAuditRun`). Contact-form records store public form URL/metadata only. Delivery events store provider message IDs, event types, timestamps, and minimal safe metadata — not full webhook payloads or email bodies. Competitor audits store Website Growth Audit snapshots for human-selected competitor websites only; they are not `AuditReport` records and are not exposed on public report pages, PDFs, Professional APIs, Stripe flows, AI Interpretation, or analytics events. These records are available only in the authenticated `/reports/prospecting` workspace.
