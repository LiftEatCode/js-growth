...
- **Storage:** PostgreSQL via Prisma (`ProspectContact`, `ProspectContactForm`, `OutreachMessage`, `OutreachDeliveryEvent`, `ProspectContactDiscoveryRun`, `ProspectOutreachDraftRun`). Contact-form records store public form URL/metadata only. Delivery events store provider message IDs, event types, timestamps, and minimal safe metadata — not full webhook payloads or email bodies. These records are available only in the authenticated `/reports/prospecting` workspace. They are not exposed on public report pages, PDFs, Professional APIs, or analytics events.
